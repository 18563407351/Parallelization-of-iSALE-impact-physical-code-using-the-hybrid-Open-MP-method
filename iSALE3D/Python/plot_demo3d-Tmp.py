...
# Open model file
m = psp.opendatfile('jdata.dat', scale='km',plottype=['Tmp'])
...
for i in np.arange(m.nsteps):
    ...
    p = ax1.pcolormesh(m.x[m.ff], m.z[m.ff], s.Tmp[m.ff],
                       cmap='viridis', vmin=np.min(s.Tmp[m.ff]), vmax=np.max(s.Tmp[m.ff]))
