...
# Open model file
m = psp.opendatfile('jdata.dat', scale='km',plottype=['Pre'])
...
for i in np.arange(m.nsteps):
    ...
    p = ax1.pcolormesh(m.x[m.ff], m.z[m.ff], s.Pre[m.ff],
                       cmap='viridis', vmin=0, vmax=1e11)
    #p = ax1.pcolormesh(m.x[m.ff], m.z[m.ff], s.Pre[m.ff],
                   #cmap='viridis', vmin=np.min(s.Pre[m.ff]), vmax=np.max(s.Pre[m.ff]))
    # Plot topography
