...
# Open model file
m = psp.opendatfile('jdata.dat', scale='km',plottype=['VEL'])
...
for i in np.arange(m.nsteps):
    ...
    p = ax1.pcolormesh(m.x[m.ff], m.z[m.ff], s.VEL[m.ff],
                       cmap='viridis', vmin=np.min(s.VEL[m.ff]), vmax=np.max(s.VEL[m.ff]))
