...
# Open model file
m = psp.opendatfile('jdata.dat', scale='km',plottype=['Yld'])
...
for i in np.arange(m.nsteps):
    ...
    p = ax1.pcolormesh(m.x[m.ff], m.z[m.ff], s.Yld[m.ff],
                       cmap='viridis', vmin=np.min(s.Yld[m.ff]), vmax=np.max(s.Yld[m.ff]))
