...
# Open model file
m = psp.opendatfile('jdata.dat', scale='km',plottype=['YAc'])
...
for i in np.arange(m.nsteps):
    ...
    p = ax1.pcolormesh(m.x[m.ff], m.z[m.ff], s.YAc[m.ff],
                       cmap='viridis', vmin=np.min(s.YAc[m.ff]), vmax=np.max(s.YAc[m.ff]))
