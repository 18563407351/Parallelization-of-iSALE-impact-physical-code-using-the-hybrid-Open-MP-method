...
# Open model file
m = psp.opendatfile('jdata.dat', scale='km',plottype=['Dam'])
...
for i in np.arange(m.nsteps):
    ...
    p = ax1.pcolormesh(m.x[m.ff], m.z[m.ff], s.Dam[m.ff],
                       cmap='viridis', vmin=np.min(s.Dam[m.ff]), vmax=np.max(s.Dam[m.ff]))
