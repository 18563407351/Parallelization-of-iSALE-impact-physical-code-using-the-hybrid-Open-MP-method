...
# Open model file
m = psp.opendatfile('jdata.dat', scale='km',plottype=['Den'])
...
for i in np.arange(m.nsteps):
    ...
    p = ax1.pcolormesh(m.x[m.ff], m.z[m.ff], s.Den[m.ff],
                       cmap='viridis', vmin=np.min(s.Den[m.ff]), vmax=np.max(s.Den[m.ff]))
