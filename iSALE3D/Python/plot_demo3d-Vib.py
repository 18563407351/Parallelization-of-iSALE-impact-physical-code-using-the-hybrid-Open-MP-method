...
# Open model file
m = psp.opendatfile('jdata.dat', scale='km',plottype=['Vib'])
...
for i in np.arange(m.nsteps):
    ...
    p = ax1.pcolormesh(m.x[m.ff], m.z[m.ff], s.Vib[m.ff],
                       cmap='viridis', vmin=np.min(s.Vib[m.ff]), vmax=np.max(s.Vib[m.ff]))
