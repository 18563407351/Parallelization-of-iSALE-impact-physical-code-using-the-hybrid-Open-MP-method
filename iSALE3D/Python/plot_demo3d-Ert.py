...
# Open model file
m = psp.opendatfile('jdata.dat', scale='km',plottype=['Ert'])
...
for i in np.arange(m.nsteps):
    ...
    p = ax1.pcolormesh(m.x[m.ff], m.z[m.ff], s.Ert[m.ff],
                       cmap='viridis', vmin=np.min(s.Ert[m.ff]), vmax=np.max(s.Ert[m.ff]))
