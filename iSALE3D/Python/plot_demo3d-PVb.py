...
# Open model file
m = psp.opendatfile('jdata.dat', scale='km',plottype=['PVb'])
...
for i in np.arange(m.nsteps):
    ...
    p = ax1.pcolormesh(m.x[m.ff], m.z[m.ff], s.PVb[m.ff],
                       cmap='viridis', vmin=np.min(s.PVb[m.ff]), vmax=np.max(s.PVb[m.ff]))
